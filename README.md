...
- **数据持久化**：每次运行结果写入 DynamoDB（PK=`MONITOR#{date}`, SK=`T#{HH:MM}`），含 5min 总量、daily 累计、各模型按 token 类型拆分明细（input/output/cache_read/cache_write），3 天 TTL 自动过期
...
发送行为：超时 10s，失败自动重试 2 次（间隔 1s）；未知 type 值 fallback 为飞书格式。
