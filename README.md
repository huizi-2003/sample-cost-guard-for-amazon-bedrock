...
- 对账口径为 UTC 日（CE DAILY 粒度按 UTC 解释），查 T-2 数据（CE 账单 T+1 才完整）
- CloudWatch 查询的 Region 从 CE 账单的 USAGE_TYPE 前缀自动推导（账单里有哪些区域就查哪些），无需手动配置
- 按模型拆分费用明细，每模型展示 input/output/cache_read/cache_write token 量
- 对账结果 + CE 原始明细 + CW 各 region 明细 全部存入 DynamoDB
- 每日推送报告（含 token 对账差异 + 费用明细），不管差异多少都推
...
═══════════════════════════════════════════════════════════════
  ...
  模型名提取 (normalize_model_name)：
    ...
    2. 去掉末尾后缀（长后缀优先匹配）：
       ...
       -cache-read-tokens-global-standard
       -input-tokens-global-standard
       ...

  Token 类型判断 (get_token_type)：
    含 "cache-read" / "cacheread" → cache_read
    含 "cache-write" / "cachewrite" → cache_write
    含 "output" → output
    其余 → input
...
═══════════════════════════════════════════════════════════════
  ...
  Region 来源：从 CE 账单 USAGE_TYPE 前缀自动推导
...
═══════════════════════════════════════════════════════════════
  ...
  SK = {model_name}  → 每模型聚合数据
    ...
    tokens_input_1k, tokens_output_1k, tokens_cache_read_1k, tokens_cache_write_1k
    estimated_cost, estimated_input_cost, estimated_output_cost, diff_pct (有单价表时)
...
用 DDB 中维护的单价表（$/1M tokens）计算估算费用，与 CE 实际费用对比：

```
estimated_cost = quantity_1K × price_per_1M / 1000

diff_pct = (actual - estimated) / estimated × 100
```

如果 diff% 大，说明单价表需要更新（AWS 调价、Savings Plans 折扣等）。

### 3. Web Console — 管理界面

- **对账看板**：
  - 按日期查看每模型费用 + token 用量（input/output/cache_read/cache_write）
  - 可展开 CE 原始明细（每条 USAGE_TYPE 的 cost、quantity、unit）
  - 可展开 CW 各 region 明细（每 region 的 token 总量）
- **配置管理**：阈值、监控 Region 列表、单价表（增删改）、Webhook 设置
...
│                              ▲                               │
│                              │                               │
│  API Gateway (REST) ──→ Lambda: web (Flask + mangum)         │
...
| 数据存储 | DynamoDB 单表 | 配置 + 阈值 + 单价 + 对账记录 + 告警状态 |
| Web 管理 | Flask + mangum + 原生 HTML/JS | API Gateway 代理到 Lambda |
...
aws cloudformation deploy \
  ...
  --stack-name bedrock-lite-guard \
  --parameter-overrides AllowedCidrs=1.2.3.4/32 \
  --capabilities CAPABILITY_IAM

# 查看部署结果（获取 Web Console URL）
aws cloudformation describe-stacks --stack-name bedrock-lite-guard \
...
aws cloudformation deploy --template-file template.yaml --stack-name bedrock-lite-guard \
  --parameter-overrides Version=2 AllowedCidrs=1.2.3.4/32 --capabilities CAPABILITY_IAM

# 手动触发对账（不用等定时任务）
aws lambda invoke --function-name bedrock-lite-guard-reconciler --region us-east-1 /dev/null

# 删除所有资源
aws cloudformation delete-stack --stack-name bedrock-lite-guard
```
