...
纯 Serverless，本身没有常驻资源，运行成本很低。下表基于**实际账单实测**（默认 10 个监控 Region，稳态运行）：

| 组件 | 用量 | 月成本（约） | 说明 |
|------|------|------------|------|
| **CloudWatch GetMetricData** | ~5.1 万 metrics/天 | **~$15** | **主要成本**。monitor 每 5 分钟 × 10 Region 拉指标，按返回指标条数计费（$0.01/1000 条） |
...
> 注：具体金额随账号的 Region 数、模型数量浮动，请以自己账单为准。上表为 2026-07 一周实测的代表值。
